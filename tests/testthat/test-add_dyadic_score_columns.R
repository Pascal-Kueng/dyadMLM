test_that("DSM constructs directional cross-sectional predictor scores", {
  ...
  expect_equal(result$.dsm_role_contrast, c(-0.5, 0.5, 0.5, -0.5))
  expect_equal(result$.x_dyad_mean_gmc, c(-2.5, -2.5, 2.5, 2.5))
  expect_equal(result$.x_within_dyad_diff, rep(4, 4))
  expect_false(".x_within_dyad_dev" %in% names(result))
  expect_false(any(startsWith(names(result), ".y_")))

  meta <- attr(result, "dyadMLM")
  expect_equal(meta$dsm_role_order, c("female", "male"))
  expect_equal(meta$dsm_role_contrast_column, ".dsm_role_contrast")
  expect_equal(
    meta$dsm_predictors,
    tibble::tibble(
      predictor = "x",
      ...
      lag = 0L,
      source_column = "x",
      mean_column = ".x_dyad_mean_gmc",
      difference_column = ".x_within_dyad_diff",
...
test_that("reversing DSM role order reverses directional columns only", {
  ...
  expect_equal(
    male_minus_female$.x_dyad_mean_gmc,
    female_minus_male$.x_dyad_mean_gmc
  )
  expect_equal(
    male_minus_female$.x_within_dyad_diff,
    -female_minus_male$.x_within_dyad_diff
  )
  expect_equal(
    male_minus_female$.dsm_role_contrast,
    -female_minus_male$.dsm_role_contrast
  )
})
...
test_that("DSM columns reproduce APIM fixed and random effects", {
  ...
  mu_x <- unique(
    (result$.x_actor + result$.x_partner) / 2 -
      result$.x_dyad_mean_gmc
  )
  ...
  apim_prediction <- ifelse(
    result$role == "female",
    apim[["alpha_f"]] + apim[["A_f"]] * result$.x_actor +
      apim[["P_f"]] * result$.x_partner,
    apim[["alpha_m"]] + apim[["A_m"]] * result$.x_actor +
      apim[["P_m"]] * result$.x_partner
  )
  dsm_prediction <-
    dsm[["a10"]] +
    dsm[["a11"]] * result$.x_dyad_mean_gmc +
    dsm[["a12"]] * result$.x_within_dyad_diff +
    result$.dsm_role_contrast * (
      dsm[["a20"]] +
        dsm[["a21"]] * result$.x_dyad_mean_gmc +
        dsm[["a22"]] * result$.x_within_dyad_diff
    )
...
test_that("DSM requires both predictor values for dyadic scores", {
  ...
  expect_true(all(is.na(result$.x_dyad_mean_gmc[result$dyad_id == 1])))
  expect_true(all(is.na(result$.x_within_dyad_diff[result$dyad_id == 1])))
  expect_equal(result$.x_within_dyad_diff[result$dyad_id == 2], c(4, 4))
})
...
test_that("DSM creates a role contrast without predictors", {
  ...
  expect_equal(generated$column, ".dsm_role_contrast")
  ...
  expect_false(any(grepl(".{pred}_dyad_mean", printed, fixed = TRUE)))
})
...
test_that("DSM keeps multiple predictors and metadata aligned", {
  ...
  expect_equal(result$.x_within_dyad_diff, rep(4, 4))
  expect_equal(result$.stress_level_within_dyad_diff, c(6, 6, 8, 8))
...
test_that("DSM constructs longitudinal raw, CWP, and CBP scores", {
  ...
  expect_equal(result$.x_dyad_mean_gmc, rep(c(-4, -1, 1, 4), each = 2))
  expect_equal(result$.x_within_dyad_diff, rep(c(2, 4, 4, 6), each = 2))
  expect_equal(result$.x_cwp_dyad_mean, rep(c(-1.5, -1.5, 1.5, 1.5), 2))
  expect_equal(result$.x_cwp_within_dyad_diff, rep(c(-1, -1, 1, 1), 2))
  expect_equal(result$.x_cbp_dyad_mean, c(rep(-2.5, 4), rep(2.5, 4)))
  expect_equal(result$.x_cbp_within_dyad_diff, c(rep(3, 4), rep(5, 4)))
  expect_equal(result$.dsm_role_contrast, rep(c(0.5, -0.5), 4))
  expect_false(any(grepl("within_dyad_dev", names(result), fixed = TRUE)))
...
test_that("DSM returns missing CWP scores for an incomplete predictor pair", {
  ...
  expect_true(all(is.na(result$.x_dyad_mean_gmc[incomplete_occasion])))
  expect_true(all(is.na(result$.x_within_dyad_diff[incomplete_occasion])))
  expect_false(any(is.na(result$.x_dyad_mean_gmc[complete_occasion])))
  expect_false(any(is.na(result$.x_within_dyad_diff[complete_occasion])))
  expect_true(all(is.na(result$.x_cwp_dyad_mean[incomplete_occasion])))
  expect_true(all(is.na(result$.x_cwp_within_dyad_diff[incomplete_occasion])))
  expect_false(any(is.na(result$.x_cwp_dyad_mean[complete_occasion])))
  expect_false(any(is.na(result$.x_cwp_within_dyad_diff[complete_occasion])))
  expect_false(any(is.na(result$.x_cbp_dyad_mean[result$dyad_id == 1])))
  expect_false(any(is.na(result$.x_cbp_within_dyad_diff[result$dyad_id == 1])))
})
