...
package_citation <- citation("dyadMLM")
citation_text <- sub(" \\(Version [^)]+\\)", "", package_citation$textVersion)
cat(sub(
  package_citation$title, paste0("*", package_citation$title, "*"),
  citation_text, fixed = TRUE
))
```

<details>
<summary>BibTeX (without version)</summary>

```{r citation-bibtex, echo = FALSE, results = 'asis'}
package_citation$key <- "dyadMLM"
package_citation$note <- NULL
cat("```bibtex\n")
print(toBibtex(package_citation))
cat("```\n")
```
...
Prepared data can be used with **glmmTMB**, **brms**, and other multilevel modelling
packages that support the required model structure. See the
...
```{r cross-sectional-check, fig.width = 10, fig.height = 6.5, fig.alt = "Six predictive-check histograms. Gender-specific SDs and partner correlation are in the top row; dyad-average and half-difference summaries are in the bottom row. Red lines mark observed values and dashed lines mark the middle 95 percent of simulations."}
...
The function groups the six plots in one figure. Use `panels = FALSE` to show
them separately.
...
With mixed dyad compositions, each observed role pair gets its own figure.
It can be worthwhile to check plausible role distinctions even when fitting an
exchangeable model, since pooling can hide differences in variances or partner
correlations. These checks alone do not establish whether fixed effects should be
pooled ([nested model comparisons](https://pascal-kueng.github.io/dyadMLM/reference/compare_nested_models.html)
can help assess those restrictions).

## Vignettes and examples

For an overview of the available functions, see the
[function reference](https://pascal-kueng.github.io/dyadMLM/reference/index.html).
