...
mixed-effects models. It supports the Actor–Partner Interdependence Model,
Dyad–Individual Model, and Dyadic Score Model parameterizations and includes
selected post-estimation tools. Models are fitted using packages such as
`glmmTMB` or `brms`.
...
- experimental predictive checks for cross-sectional `glmmTMB` models with
  Gaussian, Poisson, negative-binomial, Tweedie, Gamma, or beta responses
  (development version)

These checks ask whether a model reproduces how much partners' responses vary
and how they relate. Use `simulate_dyad_responses()` to generate new datasets
and `check_partner_dependence()` to compare them with the observed data.
The comparisons are descriptive; close agreement alone does not establish
good fit. See `?check_partner_dependence` for a complete example and supported
models.
