...
generates an arbitrary member-difference column, named `.dy_diff_*`, that is
...
We will now fit the model and then use the `dyadMLM::exchangeable_rescov()`
...
We use the `dyadMLM::exchangeable_rescov()` to recover the interpretable
...
[`dyadMLM::compare_dyad_models()`](../reference/compare_dyad_models.html)
...
$\widetilde{u}_{\mathrm{actor},j}$ the corresponding `.dy_diff_*` random slope.
...
Because the shared and `.dy_diff_*` blocks are fitted as separate random-effects
