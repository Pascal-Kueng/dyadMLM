...
### Explanation of back-transformation
If their random effects are denoted by $u_{\mathrm{mean},j}$ and
...
The helper `interdep::exchangeable_rescov()` performs this back-transformation
for every matched shared/difference block. With columns generated by
`interdep`, automatic matching is usually sufficient:

```r
interdep::exchangeable_rescov(model)
```

For a longitudinal `glmmTMB` model, the block pairs can instead be stated
explicitly. The following call recovers the stable dyad-level random-intercept
covariance and the same-occasion partner residual covariance separately:

```r
interdep::exchangeable_rescov(
  model,
  pairs = list(
    dyad = list(
      shared = "us(1 | coupleID)",
      difference = "us(0 + .i_diff_assumed_exchangeable_arbitrary | coupleID)",
      difference_indicator = ".i_diff_assumed_exchangeable_arbitrary"
    ),
    same_occasion = list(
...
were imposed when fitting the model. For example, if the entire stable
member-difference block was omitted, specify that omission explicitly:
...
Here, `NULL` does not constrain or modify the fitted model. It tells the helper
that the block was absent when the model was fitted. The missing difference
variance is represented by a structural zero, so the two members have
identical stable random intercepts. Likewise, a slope present in only one of a
selected pair of blocks is treated as structurally absent from the other.
