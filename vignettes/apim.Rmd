...
the analysis sample. Raw predictor scores could instead be used if zero is
meaningful for that variable in the analysis sample; this would change the
intercept reference point, but not the actor or partner slopes.
...
`prepare_dyad_data()` initially generated the actor and partner columns on the
original predictor scale. The lines above subtract one pooled mean from both
analysis columns, which can now be used directly in the model formula. Here is
a simple example:
