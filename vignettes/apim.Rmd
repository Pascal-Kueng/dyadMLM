...
same-occasion covariance. This full-data update is computationally demanding,
so it is shown but not run during vignette builds. Check convergence before
interpreting a fitted model.

```{r specify-ild-distinguishable-ar1, eval = FALSE}
ild_distinguishable_ar_model <- update(
  ild_distinguishable_model,
  formula = . ~ . +
...
When fitted, the two AR standard deviations and correlations describe each
role's persistent component. They are not correlations of the complete residual
...
```{r specify-ild-exchangeable-ar1, eval = FALSE}
ild_exchangeable_ar_model <- update(
  ild_apim_model,
  formula = . ~ . +
...
This full-data update is also computationally demanding and is therefore not
run during vignette builds. When fitted, its AR parameters describe persistent
within-member variation; the dyad-day blocks continue to describe same-day
partner covariance.

For the fitted concurrent base model, we can recover the member-level covariance
...
intercept-and-slope blocks. This advanced fit does not include the optional AR
update above, so its covariance estimates illustrate the parameterization
  ...
  formula = . ~ . -
    ...
    us(0 +
         ...
         .member_contrast_arbitrary:
           .provided_support_cwp_actor
       | coupleID),
  # The larger random-slope model benefits from profiling and BFGS.
