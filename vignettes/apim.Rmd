...
same-occasion blocks remain in the model. These boundary-constraint examples
are shown but not run because their convergence can depend on the optimizer and
platform; confirm convergence before interpreting either test.

```{r fit-ild-no-contrast-slope, eval = FALSE}
...
```{r backtransform-ild-no-contrast-slope, eval = FALSE}
...
```{r fit-ild-no-contrast-block, eval = FALSE}
...
```{r backtransform-ild-no-contrast-block, eval = FALSE}
