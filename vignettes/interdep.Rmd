---
title: "Preparing dyadic data with interdep"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Preparing dyadic data with interdep}
...
intensive longitudinal data, each complete dyad contributes one row per member
...
- mixed-composition dyadic MLMs that combine distinguishable and exchangeable
  ...
  [unified cross-sectional](#unified-cross-sectional-gaussian-model) and
  [unified intensive longitudinal](#unified-intensive-longitudinal-gaussian-model)
  examples.
...
Note that the current examples only include actor effects and not partner effects.
...
Note that if we supply `predictors`, for example as `c(pred1, pred2)` or as the
single predictor above, they will be centred automatically within and between person.
This behavior can be controlled via the `centering` argument. 
...
ild_distinguishable_model <- glmmTMB(
  closeness ~ 0 + 
    ...
    .i_is_female_x_male_female:.i_provided_support_cwp + 
    .i_is_female_x_male_male:.i_provided_support_cwp +
    
    # Gender-specific between-person actor effects
    .i_is_female_x_male_female:.i_provided_support_cbp + 
    .i_is_female_x_male_male:.i_provided_support_cbp +
...
ild_exchangeable_model <- glmmTMB(
  closeness ~ 
    ...
    # Pooled within-person and between-person actor effects
    .i_provided_support_cwp +
    .i_provided_support_cbp +
...
`example_dyadic_ILD_unified` contains the same three dyad compositions as the
unified cross-sectional example, but each dyad contributes repeated paired
...
This first unified ILD model includes composition-specific intercepts, fixed
time slopes, and within-person and between-person actor effects. Partner
predictor effects are left out for now. Random time slopes are also omitted;
with separate stable and same-day dyadic covariance blocks, random time slopes
add many weakly identified parameters and are a likely first source of
convergence problems.

For this maximal covariance example, `glmmTMB`'s default optimizer can report
false convergence even when the Hessian is positive definite. The `BFGS`
optimizer is used here because it reaches the same solution cleanly for this
simulated dataset.

```{r fit-unified-ild-gaussian, eval = has_glmmTMB}
...
unified_ild_gaussian_model <- glmmTMB(
  closeness ~ 0 +
    ...
    .i_is_female_x_male_female:.i_provided_support_cwp +
    .i_is_female_x_male_male:.i_provided_support_cwp +
    .i_is_female_x_female:.i_provided_support_cwp +
    .i_is_male_x_male:.i_provided_support_cwp +

    # Composition-specific between-person actor effects
    .i_is_female_x_male_female:.i_provided_support_cbp +
    .i_is_female_x_male_male:.i_provided_support_cbp +
    .i_is_female_x_female:.i_provided_support_cbp +
    .i_is_male_x_male:.i_provided_support_cbp +
...
between-person actor effects. The dyadic random-effect blocks are latent effects
on the log-mean scale, not residual covariance parameters in the Gaussian sense.

```{r fit-unified-ild-tweedie, eval = has_glmmTMB}
