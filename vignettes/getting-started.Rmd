...
Post processing functions of `dyadMLM`, including model comparison and
back-transforming the exchangeable random-effect covariance structures, are 
described in the [Actor-Partner Interdependence  Vignette](https://pascal-kueng.github.io/dyadMLM/articles/apim.html).

Other vignettes cover the [Dyad-Individual Model](dim.html) and its' equivalence
and back-transformation to the *exchangeble APIM*, and the 
[Dyadic Score Model](dsm.html) with its' equivalence and back-tranfsformation
...
<!-- TODO: insert example code for reshaping? -->

## Data preparation for distinguishable dyads

`dyads_cross` is a simulated cross-sectional dataset that contains three dyad compositions. 
...
achieved by omitting `role`. For more control on compositions, refer to later 
sections in (LINK TO THERE). <!--TODO: complete this -->
...
To compute these columns, a explicit role order is required. 
...
Note that observed person means used to construct the between-person (`cbp`) predictors can be unreliable when each member contributes few occasions, which 
can bias between-person estimates [@gottfredsonStraightforwardApproachCoping2019].

For fitted examples of intensive longitudinal dyadic models and how 
autocorrleation can be modelled for distinguishable and exchangeable dyads, 
refer to <!--TODO: insert link to vignette section -->
...
We can list all compositions that should be treated as exchangeable in 
`set_exchangeable_compositions`. The composition-type is retained. 
Note that simply omitting `role` is different, as it would pool all dyad 
compositions into one exchangeable composition and not lead to separate 
contrasts per exchangeable dyad-composition, but a single global contrast.
<!-- TODO: clearer explanation here -->
