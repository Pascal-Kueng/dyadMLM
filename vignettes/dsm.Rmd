...
- `.dy_provided_support_within_dyad_diff` $= X_{\mathrm{female}} - X_{\mathrm{male}}$
...
- `.dy_provided_support_within_dyad_diff` reverses because the predictor difference
  reverses.

- `.dy_dsm_role_contrast` reverses because the represented outcome difference
  reverses.

- `.dy_provided_support_dyad_mean_gmc:.dy_dsm_role_contrast` reverses because only
  the outcome difference reverses.

- `.dy_provided_support_within_dyad_diff:.dy_dsm_role_contrast` remains unchanged
  because both differences reverse.

The intercept and `.dy_provided_support_dyad_mean_gmc` also remain unchanged. For
the random effects, the variances of `(Intercept)` and
`.dy_dsm_role_contrast` remain unchanged, whereas their covariance reverses
sign.
...
b0_female <- apim_coef[[".dy_is_female"]]
b0_male <- apim_coef[[".dy_is_male"]]
