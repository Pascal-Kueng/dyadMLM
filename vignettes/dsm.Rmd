...
the partner correlation by comparing observed and simulated data. We supply the
original roles because the model uses a role contrast instead of `gender`.

```{r check-cross-dsm, eval = has_glmmTMB, echo = -1, fig.width = 8, fig.height = 9, out.width = "100%", fig.alt = "Six predictive-check histograms for the DSM. Member SDs and partner correlation are on the left; dyad-average and half-difference summaries are on the right. Red lines mark observed values."}
...
The left column shows role-specific SDs and partner correlation. The right
