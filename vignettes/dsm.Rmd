...
For predictor values $X_{\mathrm{female}}$ and $X_{\mathrm{male}}$, `dyadMLM`
then creates:
...
- `.dy_communication_within_dyad_diff` $= X_{\mathrm{female}} - X_{\mathrm{male}}$
...
- `.dy_communication_within_dyad_diff` reverses because the predictor difference
  reverses.

- `.dy_dsm_role_contrast` reverses because the represented outcome difference
  reverses.

- `.dy_communication_dyad_mean_gmc:.dy_dsm_role_contrast` reverses because only
  the outcome difference reverses.

- `.dy_communication_within_dyad_diff:.dy_dsm_role_contrast` remains unchanged
  because both differences reverse.

The intercept and `.dy_communication_dyad_mean_gmc` also remain unchanged. For
...
mu_X <- mean(
  (cross_dsm_data_inverted$.dy_communication_actor +
     cross_dsm_data_inverted$.dy_communication_partner) / 2 -
    cross_dsm_data_inverted$.dy_communication_dyad_mean_gmc,
  na.rm = TRUE
)
...
a11_dsm <- dsm_coef[[".dy_communication_dyad_mean_gmc"]]
a12_dsm <- dsm_coef[[".dy_communication_within_dyad_diff"]]
