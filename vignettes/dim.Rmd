...
This vignette focuses on Dyad-Individual Model (DIM) predictor construction for
dyadic multilevel models and its relationship to APIM parameterizations. For
broader guidance on fitting dyadic multilevel models with `interdep`-prepared
data, especially APIMs with multiple dyad types, generalized outcomes,
...
`.i_diff_assumed_exchangeable` contrast with values `-1` and `1` within each dyad.
...
within-dyad random deviation indexed by `.i_diff_assumed_exchangeable`.
