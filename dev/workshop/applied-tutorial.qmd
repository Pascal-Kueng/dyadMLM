...
- Provided support (0–5): observed daily predictor

**Simulated teaching outcome:**

- Sedentary time in hours per day
- Plausible daily distribution with some roughness and skew
- More own and partner support tends to accompany less sedentary time
...
differences in **provided support** associated with one's own and one's
partner's **sedentary time on the same day**?]{.rq-text}
:::
:::
...
raw_ild_dyad_data <- readRDS("dyadic-data.rds") |>
  dplyr::select(
    ...
    provided_support, sedentary
  ) |>
  dplyr::mutate(
    sedentary_hours = sedentary / 60
  ) |>
...
Index support and sedentary time by the same diary day $t$. For provided
support $X_{ri,t}$:
...
```{r}
#| code-line-numbers: "1-28|4|6-10|12-16|18-19|21-22|23-25|1-28"
#| output: false
...
ild_apim_distinguishable_model <- glmmTMB::glmmTMB(
  sedentary_hours ~
    ...
    # Stable dyad/member random effects (optionally, add slopes)
    (0 + .is_female + .is_male | couple_id) +
...
## Exchangeable ILD APIM {.smaller}

```{r}
#| output-location: slide

ild_apim_exchangeable_model <- glmmTMB::glmmTMB(
  sedentary_hours ~
    1 +
...
summary(ild_apim_exchangeable_model)
```

## No clear fit improvement from gender-specific parameters {.smaller}

```{r}
dyadMLM::compare_nested_models(
  ild_apim_distinguishable_model,
  ild_apim_exchangeable_model
)
```

- Exchangeability constraints compatible with these data
- More parsimonious exchangeable model retained for diagnostics

## Simulate residuals from the exchangeable ILD model

```{r}
#| label: simulate-dharma-ild
#| cache: true
#| cache.extra: !expr digest::digest(list(stats::logLik(ild_apim_exchangeable_model), stats::model.frame(ild_apim_exchangeable_model), body(simulate_ild_dharma)))

ild_apim_dharma_residuals <- simulate_ild_dharma(
  model = ild_apim_exchangeable_model,
...
the lag-1 check tests remaining consecutive-day dependence.
:::
...
plot(
  ...
  title = "Block-whitened DHARMa residual"
)
```

## Check consecutive-day residual dependence
...
- **For the simulated sedentary outcome:** One support point above usual was
  associated with about 8 fewer own and 4 fewer partner sedentary minutes
- **Between persons:** No clear actor or partner associations
- **Distinguishability:** Exchangeability constraints compatible with these
  data; more parsimonious exchangeable model retained
- **Diagnostics:** No clear model misfit or evidence of remaining lag-1
  dependence
- **Teaching example:** Estimates illustrate the workflow, not substantive
  study findings
...
## Decompose provided support manually [← Data preparation](#prepared-ild-apim-data){.appendix-back-link} {#manual-ild-apim-preparation .smaller}

```{r}
#| eval: false

support_components <- raw_ild_dyad_data |>
  dplyr::group_by(person_id) |>
  dplyr::mutate(
    .person_mean = mean(provided_support, na.rm = TRUE),
    .provided_support_cwp = provided_support - .person_mean
  ) |>
  dplyr::ungroup() |>
  dplyr::mutate(
    .provided_support_cbp = .person_mean -
...
partner_components <- support_components |>
  dplyr::transmute(
    couple_id,
    diaryday,
    gender = dplyr::recode(gender, female = "male", male = "female"),
    .provided_support_cwp_partner = .provided_support_cwp,
    .provided_support_cbp_partner = .provided_support_cbp
  )

ild_manual_data <- support_components |>
  dplyr::rename(
    .provided_support_cwp_actor = .provided_support_cwp,
    .provided_support_cbp_actor = .provided_support_cbp
  ) |>
  dplyr::left_join(
    partner_components,
