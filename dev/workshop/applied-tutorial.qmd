...
<!-- TODO: we need to insert a paird trajectories plot. We can re-use this code 
but make it ideally much shorter (maybe with the see package?) and also make it
prettier. 
...
## The ILD APIM: Two member-specific equations
...
## Fitting the distinguishable ILD APIM in glmmTMB
...
```
<!-- TODO: no plot is being printed! Please fix. Also, needing multiple 
custom functions here is a bit intransparent. Cans' we use raw code for the
AR test? or is it more than a handful of lines? -->
...
<!-- TODO: insert interpretation of each parameter please. what does 
a couple-level std.dev intercept of 0.23 mean? and the correlation of 0.34? what does the
daily level SD intercept of 0.35 mean, and the 0.38 corr? in applied terms. -->
...
:::{.subtle}
The correlation need not become zero. They should be typical under simulations
...
We exponentiate estimates (due to log-transformed outcome): Incidence rate ratios (IRR)

<!-- TODO: fix to the correct term, it is not IRR -->
...
ild_apim_focal_effects <- parameters::model_parameters(
  ...
  dplyr::mutate(
    ...
    estimate_percent = 100 * (exp(Coefficient) - 1),
...
ild_apim_focal_effects |>
  ...
  dplyr::transmute(
    ...
    Effect,
    `Outcome role`,
    `Estimated MVPA difference` = sprintf("%+.1f%%", estimate_percent),
    `95% CI` = sprintf(
      "%+.1f%% to %+.1f%%",
      ci_low_percent,
      ci_high_percent
...
ild_apim_focal_effects |>
  ...
  labs(
    x = "Estimated difference in current-day MVPA (%)",
    y = NULL,
    color = NULL
  ) +
  see::theme_modern(base_size = 16) +
...
```
<!-- TODO: doesn't the package we use for plotting results in the cross-sectional example
already provide functions to plot like this with much less code?? -->

:::{.text-smaller}
In this sample, one-point above-usual own and partner PBC on the previous day
were associated with about 3.5% and 2.5% higher current-day MVPA among men,
respectively; the corresponding associations among women and all between-person
associations were not clear.
:::
...
pbc_components <- raw_ild_dyad_data |>
  ...
  dplyr::mutate(
    .person_mean = mean(perceived_behavioral_control, na.rm = TRUE),
    .perceived_behavioral_control_cwp =
      perceived_behavioral_control - .person_mean,
    .perceived_behavioral_control_cwp_lag1 = dplyr::if_else(
      diaryday == dplyr::lag(diaryday) + 1,
      dplyr::lag(.perceived_behavioral_control_cwp),
      NA_real_
    )
  ) |>
  dplyr::ungroup() |>
  dplyr::mutate(
    .perceived_behavioral_control_cbp = .person_mean -
      mean(
        .person_mean[
...
partner_components <- pbc_components |>
  dplyr::transmute(
    ...
    diaryday,
    gender = dplyr::recode(gender, female = "male", male = "female"),
    .perceived_behavioral_control_cwp_partner =
      .perceived_behavioral_control_cwp,
    .perceived_behavioral_control_cwp_partner_lag1 =
      .perceived_behavioral_control_cwp_lag1,
    .perceived_behavioral_control_cbp_partner =
      .perceived_behavioral_control_cbp
  )

ild_manual_data <- pbc_components |>
  dplyr::rename(
    .perceived_behavioral_control_cwp_actor =
      .perceived_behavioral_control_cwp,
    .perceived_behavioral_control_cwp_actor_lag1 =
      .perceived_behavioral_control_cwp_lag1,
    .perceived_behavioral_control_cbp_actor =
      .perceived_behavioral_control_cbp
  ) |>
  dplyr::left_join(
    partner_components,
