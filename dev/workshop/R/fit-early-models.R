...
fit_and_draw_distinguishable_apim <- function(data, predictor, outcome) {
  ...
  draw_apim_diagram("distinguishable", model = model)
  invisible(model)
}

fit_and_draw_exchangeable_apim <- function(data, predictor, outcome) {
  variables <- .early_model_variable_names(
    data, rlang::ensym(predictor), rlang::ensym(outcome)
  )
  prepared <- .prepare_early_model_data(
  ...
  draw_apim_diagram("exchangeable", model = model)
  invisible(model)
}

fit_and_draw_dim <- function(data, predictor, outcome) {
  variables <- .early_model_variable_names(
    data, rlang::ensym(predictor), rlang::ensym(outcome)
  )
  prepared <- .prepare_early_model_data(
  ...
  draw_dim_diagram(model = model, effect_unit = "couple")
  invisible(model)
}

fit_and_draw_dsm <- function(data, predictor, outcome) {
  variables <- .early_model_variable_names(
    data, rlang::ensym(predictor), rlang::ensym(outcome)
  )
  prepared <- .prepare_early_model_data(
