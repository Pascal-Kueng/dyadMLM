...
pages <- data.frame(
  ...
  guide = c(
    ...
    "Red quartiles should roughly follow the horizontal lines at 0.25, 0.50 and 0.75, usually staying within their matching blue bands. Here the horizontal axis uses the observed predictor values. With 120 observations per role, local comparisons are less precise than the overall distribution checks.",
...
writeLines(c(
  ...
  '<p><strong>Reading the residuals:</strong> PIT ranks each outcome among its model simulations: near 0 means low, near 1 means high, and 0.5 is the middle. The <em>Predicted outcome</em> axis shows the model prediction with random effects set to zero, not the observed outcome or a prediction of residuals.</p>',
