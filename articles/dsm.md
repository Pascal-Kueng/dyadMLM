...
The curved arrow $`\rho_{md}`$ is the scale-free correlation between the
outcome-mean residual and the outcome-difference residual. It is **not**
the female-male residual correlation $`\rho_{FM}`$ from the APIM.

The DSM uses the full female-minus-male difference:

``` math
e_F = r_m + \frac{1}{2}r_d,
\qquad
e_M = r_m - \frac{1}{2}r_d.
```
...
=
\frac{\operatorname{Var}(e_F)-\operatorname{Var}(e_M)}{2}.
```

For this reason, a nonzero $`\rho_{md}`$ indicates that the two roles
have different residual variances. The remaining covariance between the
partners’ residuals is

``` math
\operatorname{Cov}(e_F,e_M)
=
\operatorname{Var}(r_m)-\frac{1}{4}\operatorname{Var}(r_d).
```
