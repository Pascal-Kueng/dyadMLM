...
dyad_generated_columns <- function(meta) {
  dplyr::bind_rows(
    temporal_predictor_generated_columns(meta$temporal_decompositions),
...
validate_generated_column_plan <- function(data, column_plan) {
  ...
  if (length(duplicate_planned_names) > 0L) {
    ...
    # Metadata records the meaning of temporal, APIM, DIM, and DSM columns
    # generated earlier. Its field names differ from those in `column_plan`.
    ...
    if (nrow(existing_generated_column_metadata) > 0L) {
      ...
      # Composition columns are created before predictor metadata exists.
      # Their name still lets us report which kind of column is in the way.
      existing_column_role <- dplyr::case_when(
        startsWith(collision_name, ".dy_is_") ~ "composition_indicator",
        startsWith(collision_name, ".dy_member_contrast_") |
          startsWith(collision_name, ".dy_diff_") ~ "member_contrast",
        .default = "previously_generated"
      )
      conflicting_origin <- tibble::tibble(
        target = collision_name,
        ...
        column_role = existing_column_role
      )
    }
  }
...
temporal_predictor_generated_columns <- function(temporal_decompositions) {
  if (is.null(temporal_decompositions) ||
      nrow(temporal_decompositions) == 0) {
    return(empty_generated_columns())
  }

  temporal_decompositions <- temporal_decompositions[
    temporal_decompositions$component %in% c("cwp", "cbp") |
      temporal_decompositions$lag > 0L,
    ,
    drop = FALSE
  ]
  ...
  columns <- tibble::tibble(
    ...
    variable = temporal_decompositions$predictor,
    component = temporal_decompositions$component,
    lag = temporal_decompositions$lag,
    column_role = "temporal_component",
    column = temporal_decompositions$column,
    source_column = temporal_decompositions$predictor
  )
